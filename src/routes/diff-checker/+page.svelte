<script>
  ...
  // `?popout=` isn't consulted here at all - see DiffChecker.svelte / popout-sync.js). Flipped by
...
<svelte:head>
  <title>{satellite ? paneLabel : 'Diff Checker'}</title>
</svelte:head>
...
<main class="flex h-screen flex-col overflow-hidden bg-white dark:bg-gray-900">
  ...
  <div class="min-h-0 flex-1">
    <DiffChecker
      initialOriginal={DEFAULT_ORIGINAL}
      initialChanged={DEFAULT_CHANGED}
      onsatellite={(v, label) => {
        satellite = v;
        paneLabel = label ?? '';
      }}
