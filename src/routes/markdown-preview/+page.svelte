<script>
  ...
  // `?popout=` isn't consulted here at all - see MarkdownPreview.svelte / popout-sync.js). Flipped
  // by the tool component once it's actually confirmed satellite mode via a completed handshake.
  let satellite = $state(false);
</script>

<svelte:head>
  <title>{satellite ? 'Markdown' : 'Markdown Preview'}</title>
</svelte:head>
...
<main class="flex h-screen flex-col overflow-hidden bg-white dark:bg-gray-900">
  ...
  <div class="min-h-0 flex-1">
    <MarkdownPreview initialContent={DEFAULT_CONTENT} onsatellite={(v) => (satellite = v)} />
  </div>
</main>
