...
<main class="flex h-screen flex-col overflow-hidden bg-white dark:bg-gray-900">
  {#if !satellite}
    ...
    >
      <a
        href={resolve('/')}
        class="text-[1.125em] text-gray-500 transition-colors hover:text-blue-600 dark:text-gray-400 dark:hover:text-blue-400"
      >
        &larr; Back to Tools
      </a>
      <h1 class="font-mono text-[1.5em] font-bold tracking-tight text-gray-900 dark:text-gray-100">
        Agent Trajectory Viewer
      </h1>
      <span class="ml-auto font-mono text-[1em] text-gray-400 dark:text-gray-500">
        {__APP_VERSION__}
      </span>
    </div>
  {/if}
